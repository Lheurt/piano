...
function StaffBand({ which, notes, playheadIndex, width, narrow, showPlayhead, wrongPitch }) {
...
function GrandStaff({ notes = [], playheadIndex = 0, clef = 'grand', width = 760, narrow = false, showPlayhead = true, wrongPitch = null }) {
  ...
  return (
    <div className={'grand-staff' + (narrow ? ' narrow' : '')}>
      {showTreble && (
        <StaffBand which="treble" notes={trebleSlots} playheadIndex={playheadIndex} width={effectiveWidth} narrow={narrow} showPlayhead={showPlayhead} wrongPitch={wrongInTreble} />
      )}
      {showTreble && showBass && <div className="staff-gap" aria-hidden="true" />}
      {showBass && (
        <StaffBand which="bass" notes={bassSlots} playheadIndex={playheadIndex} width={effectiveWidth} narrow={narrow} showPlayhead={showPlayhead} wrongPitch={wrongInBass} />
      )}
    </div>
  );
}
